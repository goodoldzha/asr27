...
/*	function activate(){
	...
	function addreplyid($commentdata){
		if(isset($_POST['comment_reply_ID']))
			$commentdata['comment_parent'] = addslashes($_POST['comment_reply_ID']);
	...
	function email($id){
		...
		if((int) addslashes($_POST['comment_reply_ID']) === 0 || (int) addslashes($_POST['comment_post_ID']) === 0){
			return $id;
		}

		if($this->options['mail_notify'] != 'none'){
			$this->mailer($id,addslashes($_POST['comment_reply_ID']),addslashes($_POST['comment_post_ID']));
		}
