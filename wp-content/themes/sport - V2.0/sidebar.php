					...
					<?php
						...
						$args=array(
							...
							while ($my_query->have_posts()) : $my_query->the_post(); 
								...
								if ($type=='pre'&& $count<2){
								?>
								<h2 class="last-match-home"><?php if(($t=get_post_meta($post->ID, "home_team" , true))!='') {?><div class="home-logo"><img src="<?php bloginfo('template_directory');?>/images/teams/<?php echo $t.'.png'; ?>" width="65px" height="65px" /></div><?php _e($t,'postechin'); } else { ?><div class="home-logo"><img src="<?php echo get_post_meta($post->ID, "home_logo" , true);?>" width="65px" height="65px" /></div><?php echo get_post_meta($post->ID, "home_name" , true);?><?php } ?></h2>
								...
								<h2 class="last-match-away"><?php if(($t=get_post_meta($post->ID, "away_team" , true))!='') {?><div class="home-logo"><img src="<?php bloginfo('template_directory');?>/images/teams/<?php echo $t.'.png'; ?>" width="65px" height="65px" /></div><?php _e($t,'postechin'); } else { ?><div class="home-logo"><img src="<?php echo get_post_meta($post->ID, "away_logo" , true);?>" width="65px" height="65px" /></div><?php echo get_post_meta($post->ID, "away_name" , true);?><?php } ?></h2>
							<?php $count++;}
							endwhile;
							}
							wp_reset_query();
